...
public class WebSocketHandler
{
    ...
    private readonly IConnectSession<WebSocket> _connectSession;
    public WebSocketHandler(InputAdmin inputAdmin, HandlerHandShake handlerHandShake, IHttpContextAccessor httpContextAccessor, IConnectSession<WebSocket> connectSession)
    {
    ...
    public async Task HandleWebSocketAsync(WebSocket socket)
    {
        ...
        HandShakeResult authentication = await _handlerHandShake.HandleAsync(socket, transport, _httpContextAccessor.HttpContext);
        ...
        if (!_connectSession.Connect(_httpContextAccessor.HttpContext, socket, transport, out Session? session))
        {
            await socket.CloseAsync(WebSocketCloseStatus.ProtocolError, "User is not authenticated!", CancellationToken.None);
            return;
        }
